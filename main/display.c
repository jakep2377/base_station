...
void display_show_status(const char *mode,
    ...
    if (page == 0) {
        write_line(0, "NAV STATUS P1", cache[0], sizeof(cache[0]));
        ...
        write_line(0, "NAV DETAIL P2", cache[0], sizeof(cache[0]));
        ...
        write_line(3, "LAST ACK", cache[3], sizeof(cache[3]));
        copy_windowed_text(ack, detail, sizeof(detail), 0);
        write_line(4, detail, cache[4], sizeof(cache[4]));

        write_line(5, "SHORT STATUS", cache[5], sizeof(cache[5]));
        snprintf(line, sizeof(line), "M:%.4s W:%.4s", mode_label, wifi_label);
        write_line(6, line, cache[6], sizeof(cache[6]));
        snprintf(line, sizeof(line), "LORA:%.9s", lora_label);
        write_line(7, line, cache[7], sizeof(cache[7]));
    } else {
        write_line_ex(0, "!!! ALERT !!!", cache[0], sizeof(cache[0]), true);

        snprintf(line, sizeof(line), "WIFI:%-.10s", wifi_label);
        ...
        snprintf(line, sizeof(line), "QUEUE:%02d", queue_depth);
        write_line_ex(3, line, cache[3], sizeof(cache[3]), true);

        snprintf(line, sizeof(line), "CMD:%-.12s", cmd_label);
        ...
        copy_windowed_text(target_network, detail, sizeof(detail), 0);
        write_line_ex(6, detail, cache[6], sizeof(cache[6]), true);

        write_line_ex(7, "CHECK LINKS", cache[7], sizeof(cache[7]), true);
    }
}
